...
// 5 - Сравнение строк при нулевом символе
...
// 6 - При использовании разного регистра
...
// 2 - Тест на сравнение строк, где первая строка короче второй
...
// 3 - Тест на сравнение пустых строк
...
// 4 - Тест на сравнение строк с ограниченной длиной n
...
// 5 - Тест на сравнение строк, где первая строка короче второй и используется
// максимальная длина
...
// 1 - Тест на поиск последнего вхождения символа в строке
START_TEST(s21_strrchr_test_1) {
  const char str[] = "Mojno v zad?";
  char ch = 'o';

  char *res1 = strrchr(str, ch);
  char *res2 = s21_strrchr(str, ch);

  ck_assert_ptr_eq(res1, res2);
}
END_TEST

// 2 - Тест на поиск символа, который не содержится в строке
...
START_TEST(strtok_test_01) {
  char str[20] = "\0This is Elon\0 Musk.";
  char str2[20] = "\0This is Elon\0 Musk.";
  char str3[] = "\0";
  ck_assert_pstr_eq(strstr(str, str3), s21_strstr(str2, str3));
}
END_TEST
...
Suite *strtok_suite(void) {
  ...
  s = suite_create("\033[35m=====s21_STRTOK=====\033[0m");
  tc_core = tcase_create("StrtokCore");

  tcase_add_test(tc_core, strtok_test_01);
  suite_add_tcase(s, tc_core);
  return s;
}
...
Suite *sprintf_c_suite(void) {
  ...
  tcase_add_test(tc_core, test_sprintf_c_2);
...
Suite *sprintf_g_suite(void) {
  ...
  tcase_add_test(tc_core, test_sprintf_g_10);
  tcase_add_test(tc_core, test_sprintf_g_11);
  suite_add_tcase(s, tc_core);
...
Suite *s21_to_upper_suite(void) {
  ...
  tc_core = tcase_create("ToUpperCore");
...
Suite *s21_to_lower_suite(void) {
  ...
  tc_core = tcase_create("ToLowerCore");
...
Suite *s21_trim_suite(void) {
  ...
  s = suite_create("\033[42m-=s21_trim=-\033[0m");
  tc_core = tcase_create("TrimCore");
